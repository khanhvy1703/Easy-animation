...
public class AnimationReader {
  ...
  private static <Doc> void readTempo(Scanner s, AnimationBuilder<Doc> builder) {
    int[] tempVal = new int[3];
    String[] fieldNames = {"start", "end", "speed"};
    for (int i = 0; i < 3; i++) {
      tempVal[i] = getInt(s, "Tempo", fieldNames[i]);
    }
    builder.setTempo(tempVal[0], tempVal[1], tempVal[2]);
  }
